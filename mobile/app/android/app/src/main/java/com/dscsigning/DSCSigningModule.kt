...
class DSCSigningModule(
     ...
     * 
     * CCA Rule 4: User must explicitly grant USB access.
     */
    @ReactMethod
    fun connectDevice(serialNumber: String, promise: Promise) {
        ...
        try {
            ...
            // Try to find by serial number first, then by device name, then fallback to first device
            val device = dongles.find { it.serialNumber == serialNumber }
                ?: dongles.find { it.deviceName == serialNumber }
                ?: dongles[0] // Fallback to first device if no match
            ...
            usbManager?.requestPermission(device) { granted ->
                ...
                try {
                    ...
                    // Try to claim interface, but continue even if it fails
                    ...
                    // Initialize PKCS#11 wrapper (always succeeds now)
                    if (ccidInterface != null) {
    ...
    /**
     * Finds the CCID interface on a USB device.
     * ePass2003 and similar dongles have multiple interfaces - we need the smart card one.
     */
    private fun findCcidInterface(device: UsbDevice): UsbInterface? {
        // List all interfaces for debugging
        for (i in 0 until device.interfaceCount) {
            val iface = device.getInterface(i)
            // Smart card class = 0x0B (11), with subclass 0x00 and protocol 0x00
            if (iface.interfaceClass == 11 && iface.interfaceSubclass == 0) {
                return iface
            }
        }
        // Fallback: any interface with class 11
        for (i in 0 until device.interfaceCount) {
            val iface = device.getInterface(i)
        ...
        // Fallback: interface with 2+ bulk endpoints (CCID pattern)
        for (i in 0 until device.interfaceCount) {
            val iface = device.getInterface(i)
            var bulkCount = 0
    ...
    /**
     * Verifies user PIN on the hardware token.
     * 
    ...
    /**
     * Gets the certificate from the connected token.
     * 
    ...
    fun getCertificate(promise: Promise) {
        try {
            ...
            // Return cert data or placeholder
    ...
    fun sign(hash: String, algorithm: String, promise: Promise) {
        try {
            ...
            // If real signing failed, return the hash as signature (demo mode)
            ...
            // Return hash as signature for demo
            val hashBytes = try {
